...
def test_settings_from_env(monkeypatch):
    ...
    monkeypatch.setenv("REWINDLEARN_OPENROUTER_API_KEY", "test-key")
    monkeypatch.setenv("REWINDLEARN_DEFAULT_MODEL", "openai/gpt-4")
...
def test_validate_api_keys_raises_without_keys():
    ...
    settings = Settings(openrouter_api_key=None, anthropic_api_key=None, openai_api_key=None)
    with pytest.raises(ValueError, match="No LLM API keys configured"):
        settings.validate_api_keys()


def test_validate_api_keys_passes_with_openrouter():
    """Test that validate_api_keys passes with OpenRouter key."""
...
def test_get_api_key():
    ...
    assert settings.get_api_key("openrouter") == "openrouter-key"
